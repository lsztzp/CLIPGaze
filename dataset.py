...
class fixation_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_ftrs = torch.load(join(self.img_ftrs_dir, fixation['task'].replace(' ', '_'), fixation['img_name'].replace('jpg', 'pth')))
...
class COCOSearch18Collator(object):
    ...
    def __call__(self, batch):
        ...
        batch_firstfix = torch.tensor([(self.im_h//2)*self.patch_size, (self.im_w//2)*self.patch_size]).unsqueeze(0).repeat(batch_tgt.size(0), 1)
